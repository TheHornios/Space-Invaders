...
public class Nave extends ObjetosJuego{
	private EstadoJuego_EsterEgg estadoJuego;
	private long tiempo;
	 ...
	 */
	public Nave(Vector2D a, BufferedImage b,EstadoJuego_EsterEgg e) {
		super(a, b);
		this.estadoJuego =e;
		tiempo=1000;
	 ...
	 *@see EstadoJuego_EsterEgg.DisparoNave
	 *@see EstadoJuego_EsterEgg.DisparosMarcianos
	...
	public void update() {
		...
		if(vidas>0){
			...
			if(Teclado.disparar && tiempo>1500){
				estadoJuego.getObjetosNave().add(new DisparoNave(getCentre(),Assets.disparoMarcianos1,Assets.disparoMarcianos2));
				tiempo = 0;
			}
	...
	public void draw(Graphics g) {
		...
		if(morir==true&&vidas>=0){
			...
			if(vidas>0&&morir==false){
				...
				if(tiempo2>700){
					g.drawImage(Assets.sterEggMacrinaVolt,(int)posicion.getX(),(int)posicion.getY(),null);
				}
				morir = false;
			}
		}
	...
	public void animacionMorir(Graphics g){
		...
		if(tiempo2<200&&morir){
			g.drawImage(Assets.sterEggMuerte,(int)posicion.getX(),(int)posicion.getY(),null);
		}
		if(tiempo2>200&&tiempo2<400&&morir){
			g.drawImage(Assets.sterEggMuerte1,(int)posicion.getX(),(int)posicion.getY(),null);
		}
		if(tiempo2>400&&tiempo2<600&&morir){
			g.drawImage(Assets.sterEggMuerte2,(int)posicion.getX(),(int)posicion.getY(),null);
		}

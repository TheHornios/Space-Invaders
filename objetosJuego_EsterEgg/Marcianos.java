...
public class Marcianos extends ObjetosJuego{
	 ...
	 */
	public Marcianos(Vector2D a, BufferedImage b, BufferedImage e,EstadoJuego_EsterEgg d,int columna,int ancho) {
		super(a, b);
		textura2 = e;
		estadoJuego = d;
		tiempo2= 0;
	...
	public void draw(Graphics g) {
			...
			if(tiempo3>500){
				...
				if(ancho==46){
					if(10==numero||120==numero||11==numero||121==numero){
						estadoJuego.getObjetosMarcianos().add(new DisparosMarcianos(getCentre(),Assets.disparoNave1, Assets.disparoNave2));
					}
				}
		...
		if(vidas2==1){
			...
			if(tiempo3<500){
				g.drawImage(Assets.sterEggExplosion,(int)posicion.getX(),(int)posicion.getY(),null);
			}
			if(tiempo3>500){
				g.drawImage(Assets.sterEgg2Explosion,(int)posicion.getX(),(int)posicion.getY(),null);
